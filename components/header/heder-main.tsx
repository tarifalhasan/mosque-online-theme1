  ...
  return (
    <Container>
      <header className="flex items-center justify-between gap-4">
        ...
        <div
          className={`fixed px-5 mx-0 ${
            menuOpen ? "left-0" : "left-full"
          } right-0 top-0 bottom-0 w-full h-full bg-[#D6EAF8] z-50 `}
        >
        ...
        <ul className=" hidden pt-10 lg:flex items-center gap-6 ">
          {NavLinks.map((link, index) => (
            <li key={index} className="  flex justify-center">
              <Link
                href={link.href}
                className=" font-normal text-lg  text-left hover:text-primary"
              >
                {link.name}
              </Link>
            </li>
        ...
        <div className=" hidden pt-10  lg:flex items-center gap-4">
          <button className="btn hover:bg-opacity-75 bg-[#1DBBB4]">
            Login
          </button>
          <button className="btn hover:bg-opacity-75 bg-[#1DBBB4]">
            Register
